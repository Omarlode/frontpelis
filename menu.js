const API_KEY = "api_key=49800de893daf00d2c9e840a061b3efc";
...
const searchURLtv = BASE_URL + "/search/tv?" + API_KEY + LANG;
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  const tagsElmovies = document.getElementById("tags");
  ...
  genresmovies.forEach((genre) => {
    ...
    t.addEventListener("click", () => {
      if (selectedGenreMovies.length == 0) {
        selectedGenreMovies.push(genre.id);
      } else {
        if (selectedGenreMovies.includes(genre.id)) {
          selectedGenreMovies.forEach((id, idx) => {
            if (id == genre.id) {
              selectedGenreMovies.splice(idx, 1);
            }
          });
        } else {
          selectedGenreMovies.push(genre.id);
      ...
      getMovies(
        API_URL + "&with_genres=" + encodeURI(selectedGenreMovies.join(","))
      );
      highlightSelection();
    });
    tagsElmovies.append(t);
  });
}
